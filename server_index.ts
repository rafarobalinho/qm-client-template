...
function sendAdminHtml(res: ServerResponse): void {
  try {
    if (!adminHtmlCache) {
      adminHtmlCache = readFileSync(join(DIST, "admin.html"), "utf8");
    }
    res.writeHead(200, {
      "content-type": "text/html; charset=utf-8",
    ...
    res.end("Admin UI not found: " + String(e));
  }
}

async function relayAdminCore(
  req: IncomingMessage,
  res: ServerResponse,
  method: HttpMethod,
  pathWithQuery: string,
  ...
  try {
    ...
    console.error("[admin] core relay error:", String(err));
    json(res, 502, { error: "core_unreachable", message: "core unavailable" });
  }
}

async function handleAdminRequest(req: IncomingMessage, res: ServerResponse, originalUrl: string, path: string): Promise<boolean> {
  ...
  if (adminPath.startsWith("/api/")) {
    const sub = adminPath.slice("/api/".length);
    const subBase = sub.split("?")[0].split("/")[0];
    
    // Whoami / Me
    if (subBase === "me" || subBase === "whoami") {
      json(res, 200, {
        principal: u,
        org: ORG,
        isAdmin: true,
    ...
    // Connector catalog
    if (subBase === "connector-catalog") {
      await relayAdminCore(req, res, "GET", "/v1/connectors/catalog", u);
      return true;
    }
    
    // Forward any other /admin/api/* to Core /v1/admin/*
    const queryIdx = originalUrl.indexOf("?");
    const query = queryIdx >= 0 ? originalUrl.slice(queryIdx) : "";
    const method = (req.method ?? "GET") as HttpMethod;
    const body = ["POST", "PUT", "PATCH"].includes(method) ? await readBody(req) : "";
    await relayAdminCore(req, res, method, `/v1/admin/${sub}${query}`, u, body);
    return true;
  }
  
  // 2. Static Brand Mark
  if (adminPath === "/brand-mark.svg") {
    await serveStatic(res, "/brand-mark.svg");
    return true;
  }
  
  // 3. Admin UI HTML (serves /admin, /admin/, /admin/users, /admin/scopes, etc.)
  sendAdminHtml(res);
  return true;
}
